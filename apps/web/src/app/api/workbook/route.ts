...
const DEFAULT_PROXY_TIMEOUT_MS = 280_000; // 280s — stays under Vercel's 300s limit
...
interface WorkbookPassageInput {
  passageId: string;
  sentences: string[];
}
...
function parseWorkbookBody(raw: unknown): {
  ok: true;
  passages: WorkbookPassageInput[];
  model: QuotaModel;
} | {
  ok: false;
  status: number;
  ...
  const passages: WorkbookPassageInput[] = [];
...
export async function POST(req: NextRequest) {
  ...
  if (!baseUrl || !apiKey) {
    console.error(
      `[api/workbook][${requestId}] Server is misconfigured: Missing baseUrl or apiKey.`
    );
    return jsonWithRequestId(
      requestId,
      {
