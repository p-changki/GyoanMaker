...
interface PocketVocaPassageInput {
  passageId: string;
  sentences: string[];
}
...
function getProxyRateLimitMax(): number {
  return toPositiveInt(process.env.PROXY_RATE_LIMIT_MAX, DEFAULT_PROXY_RATE_LIMIT_MAX);
}
...
function parsePocketVocaBody(raw: unknown):
  | { ok: true; passages: PocketVocaPassageInput[]; model: QuotaModel }
  | { ok: false; status: number; code: string; message: string } {
  if (typeof raw !== "object" || raw === null) {
    return { ok: false, status: 400, code: "INVALID_BODY", message: "Request body must be an object." };
  }

  const body = raw as { passages?: unknown; model?: unknown };

  if (body.model !== "flash" && body.model !== "pro") {
    return { ok: false, status: 400, code: "INVALID_BODY", message: 'model must be "flash" or "pro".' };
  }
  ...
  const passages: PocketVocaPassageInput[] = [];
  for (let i = 0; i < passagesRaw.length; i += 1) {
    const candidate = passagesRaw[i];
    if (typeof candidate !== "object" || candidate === null) {
      return { ok: false, status: 400, code: "INVALID_BODY", message: `passages[${i}] must be an object.` };
    }
...
export async function POST(req: NextRequest) {
  ...
  if (!baseUrl || !apiKey) {
    console.error(`[api/pocket-voca][${requestId}] Server is misconfigured: Missing baseUrl or apiKey.`);
    return jsonWithRequestId(
      requestId,
  ...
  try {
    ...
    if (modelQuota.remaining < passageCount) {
      return jsonWithRequestId(
        requestId,
        {
          error: {
            code: "QUOTA_EXCEEDED",
            message: parsed.model === "flash" ? "Flash usage limit exceeded." : "Pro usage limit exceeded.",
          },
        },
        { status: 429 }
      );
    ...
    const upstreamBody = { passages: parsed.passages, model: parsed.model };
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          const response = await fetch(`${baseUrl}/pocket-voca`, {
            method: "POST",
            headers: {
          ...
          try {
            ...
            if (totalUsage) {
              logUsage({
                ...
                console.error(`[api/pocket-voca][${requestId}] logUsage failed:`, err)
              );
            }
          } catch (quotaErr) {
            if (quotaErr instanceof QuotaExceededError) {
              console.warn(
                `[api/pocket-voca][${requestId}] Post-success quota apply failed model=${quotaErr.model} needed=${quotaErr.needed} available=${quotaErr.available}`
              );
            }
            console.error(
              `[api/pocket-voca][${requestId}] Failed to increment quota/log usage: ${quotaErr instanceof Error ? quotaErr.message : quotaErr}`
            );
          }
          ...
          if (!isTimeout) {
            console.error(`[api/pocket-voca][${requestId}] Proxy error: [${errName}] ${errMsg}`);
          }
          sendData({
    ...
    const errName = error instanceof Error ? error.name : "UnknownError";
    const errMsg = error instanceof Error ? error.message : String(error);
    console.error(`[api/pocket-voca][${requestId}] Unexpected error: [${errName}] ${errMsg}`);
    return jsonWithRequestId(
      requestId,
      { error: { code: "PROXY_ERROR", message: "Failed to connect to backend server." } },
      { status: 502 }
    );
  }
}
